...
class MatrixBiosamplesApi(Resource):
    def get(self, matrix_id):
        return Response(matrix_service.get_related_biosamples(matrix_id, request.args), mimetype="application/json", status=200)

class MatrixFeaturesApi(Resource):
    def get(self, matrix_id):
        return Response(matrix_service.get_related_features(matrix_id, request.args), mimetype="application/json", status=200)
